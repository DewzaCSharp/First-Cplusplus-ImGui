...
// Funktionsprototypen für FindWindowByProcessID und GetWindowByProcessName
HWND FindWindowByProcessID(DWORD processID);
HWND GetWindowByProcessName(const std::wstring& processName);

void WPM1(const std::wstring& processName) {
    int readTest = 0;

    // Hier suchen wir nach Notepad (oder einem anderen Prozess, z. B. "notepad.exe")
    HWND hwnd = GetWindowByProcessName(processName); // Unicode-Literal für "notepad.exe"
